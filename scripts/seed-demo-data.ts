...
const DEMO_MEMORIES: SeedMemory[] = [
  // Slack thread about auth bug — 2 days ago
  {
    source: 'slack',
  ...
  // Slack about deployment — 1 day ago
  {
    source: 'slack',
  ...
  // Screen captures — recent
  {
    ...
    metadata: { app_name: 'VS Code', window_title: 'auth.ts — project' },
    hoursAgo: 2,
  },
  {
    source: 'screen',
    ...
    metadata: { app_name: 'Chrome', window_title: 'PR #432 — GitHub' },
    hoursAgo: 6,
  },
  {
    source: 'screen',
    ...
    metadata: { app_name: 'Chrome', window_title: 'Stack Overflow' },
    hoursAgo: 50,
  },

  // Claude Code sessions — yesterday
  {
    source: 'claude_code',
...
async function main() {
  ...
  for (const mem of DEMO_MEMORIES) {
    ...
    console.log(`  [${mem.source}] ${mem.content.substring(0, 60)}...`);
    ...
    const { error } = await supabase.from('memories').insert({
      ...
      session_id: `demo-seed-${mem.source}`,
    });
  ...
  console.log('\n✅ Demo data seeded! Try these queries:');
  console.log('  - "auth token refresh issue"');
  console.log('  - "deployment status"');
  console.log('  - "where is the deploy key"');
  console.log('  - "what did we discuss about the auth bug"');
}
